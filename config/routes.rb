Rails.application.routes.draw do
  ...
  scope module: 'api' do
    namespace :v1 do
      ...
      resources :affiliations, only: %i[index update], defaults: { format: :json } do
        resource :downgrades, only: [:update], defaults: { format: :json }, controller: 'affiliations/downgrades'
        resource :positions, only: %i[show update], defaults: { format: :json }, controller: 'affiliations/positions'
      ...
      resources :groups, only: [:create], defaults: { format: :json } do
        resources :affiliations, only: %i[index create], defaults: { format: :json }, controller: 'groups/affiliations'
