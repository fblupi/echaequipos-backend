...
RSpec.describe Api::V1::Matches::PlayersController, type: :controller do
  ...
  describe '#create' do
    it 'creates the player' do
      expect((post :create,
                   params: { match_id: @match.id,
                             v1_match_players: { affiliation_id: @affiliation.id } }).response_code).to eq(200)
    end
    ...
    it 'does not create the player if invalid affiliation provided' do
      expect((post :create,
                   params: { match_id: @match.id, v1_match_players: { affiliation_id: 0 } }).response_code).to eq(401)
    end

    it 'does not create the player if affiliation from another group provided' do
      expect((post :create,
                   params: { match_id: @match.id,
                             v1_match_players: { affiliation_id: @other_affiliation.id } }).response_code).to eq(401)
    end

    it 'does not create the player if user is not admin' do
      expect((post :create,
                   params: { match_id: @another_match.id,
                             v1_match_players: { affiliation_id: @another_affiliation.id } }).response_code).to eq(401)
    end

    it 'does not create the player if there is already a player with the same affiliation' do
      expect((post :create,
                   params: { match_id: @match.id,
                             v1_match_players: { affiliation_id: @partner_affiliation.id } }).response_code).to eq(400)
    end
  end
end
