...
RSpec.describe Player, type: :model do
  describe 'validations' do
    ...
    it 'does not create a player with some parameter missing' do
    ...
    it 'updates the attending value of a proposal match' do
      expect { @player.update!(attendance: true) }.to_not raise_error
    end

    it 'does not update the attending value of a non proposal match' do
      @match.status = 'confirmed'
      expect { @player.update!(attendance: true) }.to raise_error(ActiveRecord::RecordInvalid)
  ...
  describe 'scopes' do
    it 'attending' do
      ...
      expect { @other_player.update!(attendance: true) }.to change(Player.attending, :count).by(1)
    end
  end
end
